...
function changeDep2(){
    var d2_id = $('#d2_id option:selected').val();
    if(d2_id!=0&&d2_id!='0') {
        $('#u2_id').html('');
        $.post("modules/tasks_stat/change_dep.php", {d_id: d2_id},
                function (data) {
                    var obj = jQuery.parseJSON(data);
                    if (obj.result == 'OK') {
                        $('#u2_id').html(obj.html);
...
function ShowStatTable(){
    ...
    var d2_id = $('#d2_id option:selected').val();
    var u2_id = $('#u2_id option:selected').val();
	//alert(limit);
	$('#table_rows').html('');
	$('#waitGear').show();
	$.post("modules/tasks_stat/show_stat.php", {date_start:date_start, date_end:date_end, u_id:u_id, d_id:d_id, status:status, u2_id:u2_id, d2_id:d2_id},
			function(data){
				//alert(data);
				var obj = jQuery.parseJSON(data);
				if(obj.result=='OK'){
